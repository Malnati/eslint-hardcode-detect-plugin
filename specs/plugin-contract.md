...
- **`callSiteExceptions`** (lista de strings, vazia por defeito): quando não vazia, um literal de string **não** entra nas ocorrências se for o **primeiro argumento** de uma `CallExpression` cujo callee, serializado como cadeia não computada (`console.log`, `logger.warn`, …) ou como identificador simples (`debug`, `print`), coincide com uma entrada da lista (formato alinhado ao campo `loggers` da regra `standardize-error-messages`). A serialização desembrulha `ChainExpression` (*optional chaining* no callee, ex.: `console?.log`) para a mesma cadeia que a forma sem `?.`. Callees computados (`console["log"]`), `import.meta`, chamadas dinâmicas (`obj[method]()`) ou outros nós que não serializam para esse formato **não** casam. Esses literais **não** são reportados nem participam do índice R2 nem da escrita R3.
...
- Com **`crossFileDuplicateDetection: true`** (defeito), activa-se a **detecção de duplicados cross-file** (índice R2; ver abaixo): quando o mesmo **valor normalizado** já foi visto noutro ficheiro processado **antes** no mesmo `lintFiles`, usa-se `messageId` `hardcodedDuplicateCrossFile` em vez dos relatórios de menor precedência quando aplicável. O **primeiro** ficheiro que introduz um valor mantém o relatório base até esse momento; os **seguintes** com o mesmo valor normalizado recebem o diagnóstico R2. Com **`crossFileDuplicateDetection: false`**, não há índice nem este `messageId`, independentemente de `remediationMode`. **`remediationMode: "r2"`** continua a implicar **sem** autofix R1 (modo efectivo como `off` para constantes no mesmo ficheiro). Autofix R2 (módulo partilhado) **ainda não** faz parte da implementação actual.
...
| `crossFileDuplicateDetection` | boolean | `true` | Índice de literais normalizados entre ficheiros na mesma execução `lintFiles` e relatório `hardcodedDuplicateCrossFile` quando aplicável. |
...
| `sharedConstantsModule` | string (opcional) | — | Glob de caminho relativo ao `cwd` do módulo/ficheiro partilhado de constantes; quando definido e o ficheiro actual casa, usa-se `hardcodedInSharedConstantsModule` (autofix R2 futuro). |
...
Os `messageId`s da regra devem emitir texto em **três partes** na mesma mensagem, em linhas separadas com prefixos canónicos: `[HCD-ERR-SENIOR]`, `[HCD-ERR-FIX]`, `[HCD-ERR-OPS]`.

**Precedência** (maior primeiro; a primeira condição verdadeira aplica-se): duplicado cross-file (`hardcodedDuplicateCrossFile`) → duplicado no mesmo ficheiro (`hardcodedDuplicateWithinFile`, quando `dedupeWithinFile` agrupa mais do que uma ocorrência) → ficheiro na zona `sharedConstantsModule` (`hardcodedInSharedConstantsModule`) → fallback de ambiente (`hardcodedEnvDefault` com `envDefaultLiteralPolicy === "report-separate"`) → genérico (`hardcoded`).

| ID | Uso |
|----|-----|
| `hardcoded` | Literal reportável genérico (inclui fallback de ambiente quando `envDefaultLiteralPolicy` não é `report-separate`, e após exclusão das linhas de precedência acima). |
| `hardcodedEnvDefault` | Literal de fallback de `process.env` quando `envDefaultLiteralPolicy === "report-separate"` (excepto quando há duplicado cross-file ou intra-ficheiro, ou zona `sharedConstantsModule` — prevalecem esses `messageId`s). |
| `hardcodedDuplicateCrossFile` | Com `crossFileDuplicateDetection: true`, o valor normalizado já ocorreu noutro ficheiro processado antes nesta execução `lintFiles` (ver ordem de processamento). |
| `hardcodedDuplicateWithinFile` | O mesmo valor normalizado ocorre mais do que uma vez no mesmo `SourceCode` com `dedupeWithinFile: true` (grupo com várias ocorrências). |
| `hardcodedInSharedConstantsModule` | O caminho relativo do ficheiro casa com o glob `sharedConstantsModule` (módulo ou ficheiro dedicado a constantes partilhadas); sugere externalização (propriedades, dados, env) em vez de novo literal na zona de catálogo. |
...
| `crossFileDuplicateDetection` | boolean | `true` | R2 | M2 | **Sim** (schema) | Índice global no âmbito do `lintFiles` e relatório de duplicados cross-file; desactivável sem alterar `remediationMode`. |
...
| `sharedConstantsModule` | string (glob relativo ao cwd) | — | R2 | M2 | **Sim** (opcional; mensagem dedicada; autofix futuro) | Ficheiro ou módulo de constantes partilhadas; quando o path actual casa, sugere externalização (propriedades/dados/env). |
...
- **Implementado (M2):** detecção e relatório `hardcodedDuplicateCrossFile` com `crossFileDuplicateDetection: true` (defeito) e índice por `context.settings.hardcodeDetect` (ver contrato na subsecção `no-hardcoded-strings`).
...
- **Fumaça e2e**: além do RuleTester, o pacote mantém testes de integração em `e2e/` que usam a API Node.js do ESLint (`ESLint`, `lintFiles`) contra fixtures com flat config e o plugin carregado a partir do **pacote publicado no npm** (instalado via consumidor isolado [`e2e-registry-consumer/`](../e2e-registry-consumer/) por [`e2e/run-e2e-with-registry.mjs`](../packages/eslint-plugin-hardcode-detect/e2e/run-e2e-with-registry.mjs); variável `HCD_E2E_REGISTRY_PLUGIN_ROOT`; registo local em `e2e/.e2e-registry-control.jsonl` com campo `outcome` `"100%"` / `"0%"`; deduplicação local só quando já existe `"100%"` para o mesmo commit+versão npm; fora de CI):
  ...
  - **`callSiteExceptions`** (`e2e/fixtures/call-site-exceptions/`, `call-site-baseline/`, `call-site-exceptions-r2/`): baseline sem lista vs. lista com `console.log`/`debug`, casos misto/segundo argumento/env aninhado, optional chaining, e R2 com `console.log` ignorado no índice; o e2e `call-site-exceptions.e2e.mjs` cobre o fluxo completo.
...
- **1.7.0** — `no-hardcoded-strings`: opção `crossFileDuplicateDetection` (defeito `true`); `messageId`s `hardcodedDuplicateWithinFile`, `hardcodedInSharedConstantsModule`; precedência de mensagens; `sharedConstantsModule` como glob para zona de catálogo e mensagem dedicada; detecção cross-file desacoplada de `remediationMode: "r2"`.
- **1.6.4** — e2e: registo em disco com `outcome` `"100%"` (sucesso total) ou `"0%"` (falha); abort local só com `"100%"` prévio para o mesmo commit+versão npm.
- **1.6.3** — e2e: plugin resolvido a partir do npm (`eslint-plugin-hardcode-detect@latest` no consumidor `e2e-registry-consumer/`); orquestrador `run-e2e-with-registry.mjs`, `HCD_E2E_REGISTRY_PLUGIN_ROOT`, ficheiro de controlo `.e2e-registry-control.jsonl` e deduplicação local (desativada em CI; `HCD_E2E_FORCE=1` para forçar).
- **1.6.2** — compatibilidade e2e: `call-site-exceptions.e2e.mjs` e fixtures sob `e2e/fixtures/call-site-*` para `callSiteExceptions` (baseline, detecção, R2).
- **1.6.1** — clarificação: `callSiteExceptions` desembrulha `ChainExpression` no callee (*optional chaining*, ex. `console?.log`) para a mesma cadeia que sem `?.`; mantêm-se fora de escopo callees computados e chamadas dinâmicas.
