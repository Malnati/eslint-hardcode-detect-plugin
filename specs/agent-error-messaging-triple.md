...
**Automação local (Cursor):** em workspaces de confiança, o repositório inclui o hook `.cursor/hooks/hcd-err-triple-audit.sh` (registado em `.cursor/hooks.json`). O script **acumula** ficheiros editados pelo Agent (ferramenta Write) cujos caminhos são elegíveis: sufixos habituais (entre outros, `.ts`, `.tsx`, `.js`, `.mjs`, `.cjs`, `.md`, `.mdc`, `.py`, `.pyi`, `.pyw`, `.html`, `.vue`, `.java`, `.php`, `.phtml`, `.sql`, `.ddl`, `.sh`, `.bash`) e basenames de tooling (`Dockerfile`, `Dockerfile.*`, `Makefile` / `makefile`). A lista completa está no próprio script. Aplica um **gate por regex** (só audita contagens quando há sinal de relato de falha ou mensagem de erro típica; lista configurável no script), **exclui** mecanicamente `specs/agent-error-messaging-triple.md` e o fixture de massa negativa do smoke `scripts/fixtures/smoke-hcd-err-violation-body.md` (lista `AUDIT_EXCLUDE_PATHS` no hook), valida Níveis 1–2 ao `stop`, regista em `.log/hooks/YYYYMMDD-hcd-err-audit.md`, tenta em seguida **`git add` / `git commit` / `git push`** apenas desse relatório (se o índice Git não tiver outros ficheiros staged e o repositório for um clone com `git` disponível) e pode emitir `followup_message` **sem** usar o evento `subagentStop` nem Task. Para desativar a automação Git (smoke, CI, ou clones sem remoto), use `HCD_ERR_AUDIT_SKIP_GIT=1`. A remediação referencia [`.github/agents/hcd-err-messaging.agent.md`](../.github/agents/hcd-err-messaging.agent.md) (ponte estreita; o Cursor não inicia outro agente por API — a mensagem recontinua o agente principal na mesma conversa).
...
- **2.1.6** — hook: `AUDIT_EXCLUDE_PATHS` inclui fixture de smoke negativo (`scripts/fixtures/smoke-hcd-err-violation-body.md`); smoke evita literal `exit <não-zero>` no fonte (gate regex).
- **2.1.5** — hook: mais extensões (Python, Vue/HTML, Java, PHP, SQL/DDL, shell, …), Docker/Makefile por basename; após `stop`, commit/push opcional do relatório diário; `HCD_ERR_AUDIT_SKIP_GIT`.
- **2.1.4** — `.log/hooks/` pode ser versionado (exceção a `*.log` que casava o segmento `.log`; `.log/hooks/.state/` permanece ignorado).
