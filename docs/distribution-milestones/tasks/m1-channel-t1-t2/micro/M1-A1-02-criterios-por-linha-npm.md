...
| **npm workspaces / monorepo** | Igual à linha anterior: instalação na **raiz** (`workspaces: ["packages/*"]` em [`package.json`](../../../../../package.json)); depois `npm test -w eslint-plugin-hardcode-detect`. | Mesmo job `test`; **Install** usa `npm install` na raiz. | `0` | Idem; confirma que o plugin e a fixture Nest resolvem como workspaces irmãos após install na raiz. | **Parcial** — é o cenário actual do repositório. |
