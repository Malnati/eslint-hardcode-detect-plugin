...
- **Micro-tarefas (filhas de A{N})** — quando o marco tiver pasta `tasks/<slug>/micro/` com ficheiros `M0-A<1-5>-<nn>-<slug>.md` (prefixo do marco), listar na âncora A{N} uma tabela com `micro_id` e links; manter `coverage-manifest.json` na pasta do marco (ex.: `docs/distribution-milestones/tasks/m0-baseline/coverage-manifest.json`) alinhado ao plano e validado por `scripts/validate-m0-plan-coverage.mjs`.
- **Notas**
