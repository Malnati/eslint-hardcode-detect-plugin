---
title: "Node.js API Reference - ESLint - Pluggable JavaScript Linter"
source: "https://eslint.org/docs/latest/integrate/nodejs-api#testing-errors-with-messageid"
author:
published:
created: 2026-04-11
...
- `options.ignore` (`boolean`)  
	Default is `true`. If `false` is present, the [`eslint.lintFiles()`](#-eslintlintfilespatterns) method doesn’t respect `ignorePatterns` in your configuration.
...
- `options.baseConfig` (`Config | Config[] | null`)  
	Default is `null`. [Configuration object](https://eslint.org/docs/latest/use/configure/), extended by all configurations used with this instance. You can use this option to define the default settings that will be used if your configuration files don’t configure it.
- `options.overrideConfig` (`Config | Config[] | null`)  
	Default is `null`. [Configuration object](https://eslint.org/docs/latest/use/configure/), added after any existing configuration and therefore applies after what’s contained in your configuration file (if used).
...
- `options.stats` (`boolean`)  
	Default is `false`. When set to `true`, additional statistics are added to the lint results (see [Stats type](https://eslint.org/docs/latest/extend/stats#-stats-type)).
...
- `options.applySuppressions` (`boolean`)  
	Default is `false`. If `true`, suppressions from the suppressions file are automatically applied to results from both [`eslint.lintFiles()`](#-eslintlintfilespatterns) and [`eslint.lintText()`](#-eslintlinttextcode-options). When using `eslint.lintText()`, the `filePath` option must also be provided for suppressions to take effect.
...
- `nameOrPath` (`string | undefined`)  
	...
	- `undefined`. In this case, loads the `"stylish"` built-in formatter.
		- A name of [built-in formatters](https://eslint.org/docs/latest/use/formatters/).
...
- `stats` (`Stats | undefined`)  
	The [Stats](https://eslint.org/docs/latest/extend/stats#-stats-type) object. This contains the lint performance statistics collected with the `stats` option.
- `usedDeprecatedRules` (`{ ruleId: string; replacedBy: string[]; info: DeprecatedInfo | undefined }[]`)  
	The information about the deprecated rules that were used to check this file. The `info` property is set to `rule.meta.deprecated` if the rule uses the [new `deprecated` property](https://eslint.org/docs/latest/extend/rule-deprecation).
...
- `messageId` (`string | undefined`)  
	Same as `messageId` in [LintMessage](#-lintmessage-type) type.
- `line` (`number | undefined`)  
...
- `format` (`(results: LintResult[], resultsMeta?: ResultsMeta) => string | Promise<string>`)  
	The method to convert the [LintResult](#-lintresult-type) objects to text. `resultsMeta` is an optional parameter that is primarily intended for use by the ESLint CLI and can contain `color` and `maxWarningsExceeded` properties that would be passed through the [`context`](https://eslint.org/docs/latest/extend/custom-formatters#the-context-argument) object when this method calls the underlying formatter function. Note that ESLint automatically generates `cwd` and `rulesMeta` properties of the `context` object, so you typically don’t need to pass in the second argument when calling this method.
...
- `cwd` - Path to a directory that should be considered as the current working directory. It is accessible to rules from `context.cwd` (see [The Context Object](https://eslint.org/docs/latest/extend/custom-rules#the-context-object)). If `cwd` is `undefined`, it will be normalized to `process.cwd()` if the global `process` object is defined (for example, in the Node.js runtime), or `undefined` otherwise.
...
In this example, rules run on `linter1` will get `path/to/project` from `context.cwd`. Those run on `linter2` will get `process.cwd()` if the global `process` object is defined or `undefined` otherwise (e.g. on the browser [https://eslint.org/demo](https://eslint.org/demo)).
...
- `config` - a [Configuration object](https://eslint.org/docs/latest/use/configure/) or an array of configuration objects.
	- **Note**: If you want to lint text and have your configuration be read from the file system, use [`ESLint#lintFiles()`](#-eslintlintfilespatterns) or [`ESLint#lintText()`](#-eslintlinttextcode-options) instead.
- `options` - (optional) Additional options for this run.
	- `filename` - (optional) the filename to associate with the source code.
		- `preprocess` - (optional) A function that [Processors in Plugins](https://eslint.org/docs/latest/extend/plugins#processors-in-plugins) documentation describes as the `preprocess` method.
		- `postprocess` - (optional) A function that [Processors in Plugins](https://eslint.org/docs/latest/extend/plugins#processors-in-plugins) documentation describes as the `postprocess` method.
...
- `suggestions` - an array of objects describing possible lint fixes for editors to programmatically enable (see details in the [Working with Rules docs](https://eslint.org/docs/latest/extend/custom-rules#providing-suggestions)).
...
This method is used to get the times spent on (parsing, fixing, linting) a file. See `times` property of the [Stats](https://eslint.org/docs/latest/extend/stats#-stats-type) object.

### Linter#getFixPassCount()

This method is used to get the number of autofix passes made. See `fixPasses` property of the [Stats](https://eslint.org/docs/latest/extend/stats#-stats-type) object.
...
- `config` (`Config`)  
	A [Configuration object](https://eslint.org/docs/latest/use/configure/) applied by default to all tests. It is applied before per-instance `RuleTester` constructor options and before per-test configuration. Throws a `TypeError` if `config` is not an object.
...
- The rule object itself (see [“working with rules”](https://eslint.org/docs/latest/extend/custom-rules)).
...
- `requireData` (boolean/ `"error"` / `"suggestion"`, optional):
	- If `true`, each error object that specifies `messageId` and each suggestion object that specifies `messageId` must also specify `data` if the message referenced by `messageId` has [placeholders](https://eslint.org/docs/latest/extend/custom-rules#use-message-placeholders).
		- If `"error"`, each error object that specifies `messageId` must also specify `data` if the message referenced by `messageId` has [placeholders](https://eslint.org/docs/latest/extend/custom-rules#use-message-placeholders).
		- If `"suggestion"`, each suggestion object that specifies `messageId` must also specify `data` if the message referenced by `messageId` has [placeholders](https://eslint.org/docs/latest/extend/custom-rules#use-message-placeholders).
