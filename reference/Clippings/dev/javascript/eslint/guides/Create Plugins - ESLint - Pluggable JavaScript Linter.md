---
title: "Create Plugins - ESLint - Pluggable JavaScript Linter"
...
If you plan to distribute your plugin as an npm package, make sure that the module that exports the plugin object is the default export of your package. This will enable ESLint to import the plugin when it is specified in the command line in the [`--plugin` option](https://eslint.org/docs/latest/use/command-line-interface#--plugin).
...
Plugins can expose [processors](https://eslint.org/docs/latest/extend/custom-processors) for use in configuration file by providing a `processors` object. Similar to rules, each key in the `processors` object is the name of a processor and each value is the processor object itself. Here’s an example:
...
ESLint provides the [`RuleTester`](https://eslint.org/docs/latest/integrate/nodejs-api#ruletester) utility to make it easy to test the rules of your plugin.
