...
**e2e e npm registry:** a fase e2e corre [`e2e/run-e2e-with-registry.mjs`](e2e/run-e2e-with-registry.mjs), que executa `npm install` em [`e2e-registry-consumer/`](../../e2e-registry-consumer/) (fora dos workspaces) para instalar `eslint-plugin-hardcode-detect@latest` e define `HCD_E2E_REGISTRY_PLUGIN_ROOT`. Os ficheiros `e2e/*.e2e.mjs` carregam o plugin a partir desse pacote. Execuções bem-sucedidas são registadas em `e2e/.e2e-registry-control.jsonl` (gitignored). Se o par (commit git, versão npm) já tiver sido testado com sucesso localmente, a fase e2e aborta com `[HCD-E2E-REGISTRY]`; em CI a deduplicação está desligada. Para repetir sem alterar commit: `HCD_E2E_FORCE=1` ou `HCD_E2E_SKIP_REGISTRY_DEDUP=1`.
