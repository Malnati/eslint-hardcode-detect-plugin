...
- Patch version bump after full `npm test` validation (RuleTester + all e2e via registry orchestrator); no changes to exported rules, options, or runtime behavior.
...
- RuleTester case for `console?.log` with `callSiteExceptions: ["console.log"]`.
