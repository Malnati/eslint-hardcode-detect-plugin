...
1a. [`specs/agent-error-messaging-triple.md`](specs/agent-error-messaging-triple.md) — **mensagens de falha em três partes** (diagnóstico técnico sênior, correção definitiva, contorno operacional); obrigatório ao relatar erros de ferramentas, testes, CI ou bloqueios.
...
| `.cursor/rules/` | Regras Cursor (`alwaysApply`): governança IA, sessão do agente, Clippings, documentação, Git, layout do repo, integrações sem mocks (`agent-integration-testing-policy.mdc`), caminhos relativos ao citar o repo (`repo-relative-paths.mdc`), mensagens de falha em três partes (`agent-error-messaging-triple.mdc`). |
