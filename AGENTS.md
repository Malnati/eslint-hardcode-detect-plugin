...
1c. [`specs/agent-remediation-micro-roles.md`](specs/agent-remediation-micro-roles.md) — **sub-micro-tarefas por papel** (foco único) em planos de desenvolvimento, testes, correcções e marcos `docs/*-milestones/` com `micro/`.
...
| `.cursor/rules/` | Regras Cursor (`alwaysApply`): governança IA, sessão do agente, Clippings, documentação, Git, layout do repo, integrações sem mocks (`agent-integration-testing-policy.mdc`), caminhos relativos ao citar o repo (`repo-relative-paths.mdc`), falhas com prefixos HCD-ERR-* (`agent-error-messaging-triple.mdc`), sub-micro-tarefas por papel (`agent-remediation-micro-roles.mdc`). |
| `.cursor/hooks.json` + `.cursor/hooks/` | Hooks Cursor de projeto: gate por regex + auditoria HCD-ERR (Níveis 1–2) quando há sinal de falha; várias extensões e Docker/Makefile (ver spec); registo em `.log/hooks/`; commit/push opcional do relatório (`HCD_ERR_AUDIT_SKIP_GIT=1` desliga); `followup_message` sem Task. Ver [`specs/agent-error-messaging-triple.md`](specs/agent-error-messaging-triple.md), [`.github/agents/hcd-err-messaging.agent.md`](.github/agents/hcd-err-messaging.agent.md) e smoke [`scripts/smoke-cursor-hcd-err-hook.sh`](scripts/smoke-cursor-hcd-err-hook.sh) / [`docs/cursor-vps-cli-parity.md`](docs/cursor-vps-cli-parity.md). |
| `.cursor/skills/` | Skills reutilizáveis pelos agentes neste repo (`git-agent-workflow`, `github-markdown-docs`, `eslint-plugin-workflow`, `reference-clippings-workflow`, `reference-agents-portfolio`, `docker-compose-workflow`, `agent-error-messaging-triple`, `remediation-micro-roles-workflow`). |
...
| `.github/agents/`, `.github/instructions/` | Pontes opcionais para GitHub Copilot (`eslint-hardcode-plugin`, `docker-tooling`, `hcd-err-messaging`; instruções por glob, incl. `milestones-planning` para `docs/*-milestones/**`); normas completas em `AGENTS.md` e `specs/` (ver [`specs/agent-tooling-ecosystem-map.md`](specs/agent-tooling-ecosystem-map.md)). |
